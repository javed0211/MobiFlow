...
`mobiflow init` copies the starter cases below — keep them or copy one to edit.
...
| `cases/` | Starter scenarios (Wikipedia, Joplin, Bitwarden, WDIO, cloud labs, iOS) |
| `flows/` | Frozen Maestro YAML for those samples + generated flows |
...
Starter `.txt` files are already in `cases/` (plus matching `flows/` and `data/`).
Copy one to start a new scenario:
...
def _init_noninteractive(
    *, mode: Optional[str], path: Optional[str], install_deps: bool = True
    ...
    console.print(
        "[dim]Export your API key, then: mobiflow run cases/example.txt "
        "(other starters are in cases/)[/dim]"
    )
    return out
...
def _run_init_interactive(
    *,
    ...
    case_names = sorted(p.name for p in (repo / "cases").glob("*.txt"))
    _ok(f"Sample cases → {repo / 'cases'} ({len(case_names)}: {', '.join(case_names)})")
...
def _patch_example_case(text: str, config: MobiflowConfig) -> str:
...
def _ensure_project_docs(repo: Path, config: MobiflowConfig) -> None:
    ...
    from mobiflow.samples import copy_sample_library
    ...
    example_existed = example.exists()

    try:
        copy_sample_library(
            repo,
            cases_dir=config.stack.cases_dir,
            flow_dir=config.stack.flow_dir,
        )
    except FileNotFoundError:
        pass

    if not example.exists():
        example.write_text(_patch_example_case(EXAMPLE_CASE, config), encoding="utf-8")
    elif not example_existed:
        example.write_text(
            _patch_example_case(example.read_text(encoding="utf-8"), config),
            encoding="utf-8",
        )
    ...
    if not readme.exists():
        readme.write_text(_CASES_README, encoding="utf-8")
    ...
    gitkeep = flows / ".gitkeep"
    if not gitkeep.exists() and not any(flows.glob("*.yaml")):
...
def _print_usage(repo: Path, llm: LlmConfig) -> None:
    ...
    console.print(Panel.fit(
        ...
        f"  mobiflow run cases/ --tag smoke\n\n"
        f"[dim]Guide: MOBIFLOW.md · Sample cases: cases/ · Config: mobiflow.config.yaml[/dim]",
        border_style="cyan",
    ))
